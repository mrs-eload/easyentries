...
var Axis = /** @class */function (_super) {
    ...
    function Axis(start, end, width, color, dashed) {
...
Object.defineProperty(exports, "__esModule", { value: true });
var paper = __webpack_require__(/*! paper */ "./node_modules/paper/dist/paper-full.js");
var Circle = paper.Shape.Circle;
var Color = paper.Color;
var Point = paper.Point;
var nCircle = /** @class */function (_super) {
    __extends(nCircle, _super);
    function nCircle(center, radius, color) {
        var _this = _super.call(this, center, radius) || this;
        _this.center = center;
        _this.diameter = radius * 2;
        _this.path = _this.toPath(false);
        _this.deg360 = new Point(_this.center.x, _this.center.y - radius);
        _this.deg90 = new Point(_this.center.x + radius, _this.center.y);
        _this.deg180 = new Point(_this.center.x, _this.center.y + radius);
        _this.deg270 = new Point(_this.center.x - radius, _this.center.y);
        _this.strokeColor = new Color(color);
        _this.strokeWidth = 2;
        return _this;
    }
    return nCircle;
}(Circle);
exports.nCircle = nCircle;
...
var circle_1 = __webpack_require__(/*! ./circle */ "./src/circle.js");
var axis_1 = __webpack_require__(/*! ./axis */ "./src/axis.js");
var translation_1 = __webpack_require__(/*! ./translation */ "./src/translation.js");
var canvas = document.getElementById('entry-tool-canvas');
var $std_chkbx = document.getElementById('enable_nnstd');
var $progress_bar = document.getElementById('progress');
var $entry_radial = document.getElementById('outbound_progress');
var $plane_radial = document.getElementById('plane_radial');
var $plane_heading = document.getElementById('plane_heading');
var $outbound_radial = document.getElementById('outbound_radial');
// INPUT FIELDS
var $plane_input = document.getElementById('plane-hdg-input');
var $plane_btn = document.getElementById('plane-heading-btn');
var $lang_btns = document.querySelectorAll('.switchlang');
var $outbound_input = document.getElementById('outbound-input');
var $outbound_btn = document.getElementById('outbound-btn');
var axis_radial = 90;
var plane_radial = 180;
// @ts-ignore
paper.setup(canvas);
...
var base_vector = center.subtract(circle.deg360);
var entry_vector = base_vector.clone();
var hAxis = new axis_1.Axis(circle.deg90, circle.deg270);
var yAxis = new axis_1.Axis(circle.deg360, circle.deg180);
var fixed_axis = new paper.Layer([hAxis, yAxis]);
...
var last_degree = { plane: 0, axis: 0 };
var planeText = new Group();
function setup() {
    onLangChange(null);
    ...
    $lang_btns.forEach(function ($lang) {
        return $lang.addEventListener('click', onLangChange);
    });
    updateText(last_degree.plane, 'plane');
    updateText(last_degree.axis, 'axis');
}
function onKeyBoardInput(event) {
    var to_move = event.target.dataset.obj;
    switch (event.key) {
        case "Enter":
        case "ArrowUp":
        case "ArrowDown":
...
function onProgressBarStep(event) {
    ...
    var to_move = event.target.dataset.obj;
    var $input = to_move === 'plane' ? $plane_input : $outbound_input;
...
function onStdChange(event) {
    var prefix = event.target.checked ? 'l' : 'r';
...
function onLangChange(event) {
    ...
    for (var i = 0; i < translatables.length; i++) {
        var to_trans = translatables[i];
        //@ts-ignore
        to_trans.innerHTML = translation_1.translations[lang][to_trans.dataset.trans];
    }
    updateText(last_degree.plane, 'plane');
    updateText(last_degree.axis, 'axis');
    updateEntry();
...
function draw() {
    ...
    var plane_layer = new paper.Layer();
...
function drawAxis(prefix) {
    var axis_layer = new paper.Layer();
    axis_layer.name = prefix + "_axis_layer";
    var mhAxis = new axis_1.Axis(new paper.Point(circle.deg90.x + 30, circle.deg90.y), new paper.Point(circle.deg270.x - 30, circle.deg270.y));
    mhAxis.name = prefix + "_hAxis";
    var p1 = circle.deg360.clone();
    var p2 = circle.deg180.clone();
    p1.y = p1.y - 30;
    p2.y = p2.y + 30;
    var angle = prefix === 'l' ? -20 : 20;
    var mvAxis = new axis_1.Axis(p1.rotate(angle, center), p2.rotate(angle, center));
...
function drawHoldingPattern(prefix) {
    var basic_holding = new paper.Layer();
    basic_holding.name = prefix + "_basic_holding";
    var x_modifier = prefix === 'l' ? -40 : -40;
    var y_modifier = prefix === 'l' ? 90 : -90;
    var top_left = new paper.Point(center.x + x_modifier, center.y + y_modifier);
    var bottom_right = new paper.Point(circle.deg90.x + x_modifier, circle.deg90.y);
    var rectangle = new paper.Rectangle(top_left, bottom_right);
    var radius = new paper.Size(45, 45);
    var hold = new paper.Path.Rectangle(rectangle, radius);
...
function updateEntry() {
    ...
    var text = new paper.PointText(plane.position);
    var vector = center.subtract(plane.position);
    var angle = getFullAngle(entry_vector.getDirectedAngle(vector));
    var txt_angle = getFullAngle(base_vector.getDirectedAngle(vector));
    console.log(angle);
    if (current_prefix === 'r') {
...
function showDirect() {
    for (var prefix in movable) {
        movable[prefix].entries.children.forEach(function (item) {
    ...
    return translation_1.translations[lang].direct;
}
function showTearDrop() {
    ...
    return translation_1.translations[lang].teardrop;
}
function showParallel() {
    ...
    return translation_1.translations[lang].parallel;
}
function drawDirect(prefix) {
    var rect_x_modifier = prefix === 'l' ? -40 : -40;
    var rect_y_modifier = prefix === 'l' ? 85 : -85;
    var direct = new CompoundPath('');
    var rect_top = new paper.Point(center.x + rect_x_modifier, center.y + rect_y_modifier);
    var rect_bottom = new paper.Point(circle.deg90.x + rect_x_modifier, circle.deg90.y + 1);
    var rectangle = new paper.Rectangle(rect_top, rect_bottom);
    var radius = new paper.Size(45, 45);
    var pattern = new paper.Path.Rectangle(rectangle, radius);
    var outbnd_arrow = createArrow(center.x - rect_x_modifier + 20, center.y + rect_y_modifier);
...
function drawTearDrop(prefix, direct) {
    ...
    var teardrop = new CompoundPath('');
    teardrop.name = prefix + "_teardrop";
    var start = circle.deg90.clone();
    var angle = prefix === 'l' ? 30 : -30;
    start = start.rotate(angle, center);
    var temp_segment = new paper.Path.Line(center, start);
    var inter = temp_segment.getIntersections(direct);
    var turning_point = null;
    inter.forEach(function (inters) {
        turning_point = inters.point;
    });
    var through_modifier = {
        x: prefix === 'l' ? -40 : -40,
    ...
    var teardrop_segment = new paper.Path.Line(center, turning_point);
    var through = new paper.Point(circle.deg90.x + through_modifier.x, center.y + through_modifier.y);
    var final = new paper.Point(circle.deg90.x + final_modifiers.x, circle.deg90.y + final_modifiers.y);
    var teardrop_arc = new paper.Path.Arc(turning_point, through, final);
    var inbound_seg = new paper.Path.Line(center, final);
    var outbnd_arrow = createArrow(center.x + 100, center.y);
    var inbnd_arrow1 = outbnd_arrow.clone();
    ...
    return { teardrop: teardrop, turning_point: turning_point };
}
...
function createArrow(x, y) {
    var arrow = new paper.CompoundPath('');
    var arrow_head = new paper.Point(x, y);
    var arrow_base1 = new paper.Point(arrow_head.x - 10, arrow_head.y + 8);
    var arrow_base2 = new paper.Point(arrow_head.x - 10, arrow_head.y - 8);
...
function drawEntries(prefix) {
    var entries = new paper.Layer();
    entries.name = 'entries';
    var direct = drawDirect(prefix);
    var _a = drawTearDrop(prefix, direct),
        teardrop = _a.teardrop,
        turning_point = _a.turning_point;
    var parallel = drawParallel(prefix, turning_point);
...
function getIntersections(axis, prefix) {
    var intersections = circle.path.getIntersections(axis);
    for (var i = 0; i < intersections.length; i++) {
        var point1 = intersections[i].point;
        var vector = center.subtract(point1);
        var angle = getFullAngle(base_vector.getDirectedAngle(vector));
        var text = new paper.PointText(intersections[i].point);
...
function updateAxis(rotation, degree) {
    ...
    var _loop_1 = function _loop_1(prefix) {
        rotation = degree - last_degree.axis;
        movable[prefix].axis.rotate(Math.floor(rotation), center);
        var axis_layer = movable[prefix].axis;
        var axises = axis_layer.children;
    ...
    return { degree: degree, rotation: rotation };
}
function updatePlane(degree) {
    var rotation = degree - last_degree.plane;
...
function getRadial(last, radial) {
    var final = radial + last > 360 ? radial + last - 360 : radial + last;
...
exports.translations = {
    en: {
        ...
        teardrop: "Teardrop",
        parallel: "Parallel",
        heading_error: "Heading must be between 1 and 360",
        outbound_error: "Outbound radial must be between 1 and 360",
    ...
    fr: {
        ...
        heading_error: "Le cap doit être compris entre 1 et 360",
        outbound_error: "L'axe d'éloignement être compris entre 1 et 360",
        plane_heading: "Cap avion",
        course: "QDM",
        outbound_leg: "Axe d'éloignement"
    }
};
